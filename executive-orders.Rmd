...
presidents <- read_tsv("./presidents.tsv", show_col_types = FALSE) |>
  mutate(
    term = parse_date_format(`Term Began`) %--%
      coalesce(parse_date_format(`Term Ended`), data_fetch_date),
    days_in_office = term / ddays(),
  ) |>
  select(name = `President Name`, term, days_in_office)
```
...
orders_by_presidents <- left_join(
  presidents,
  orders,
  by = join_by(name == president),
  relationship = "many-to-many", # mended in filter below
) |>
  filter(date %within% term) |>
  group_by(name) |>
  summarize(
    first_term_start = min(int_start(term)),
    num_orders = sum(!is.na(date)),
  ) |>
  filter(num_orders > 0) |>
  inner_join(
    summarize(
      group_by(presidents, name),
