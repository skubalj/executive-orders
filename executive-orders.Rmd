...
daily_order_totals <- inner_join(
  ...
  mutate(num_before_day = cumsum(num_on_day)) |>
  arrange(term_start)
```

```{r, fig.height=6, fig.width=10}
ggplot(daily_order_totals) +
  geom_line(aes(days_into_term, num_before_day, color = fct_reorder(term_id, term_start))) +
  labs(x = "Days Into Presidential Term", y = "Total Executive Orders", color = "President") +
  scale_x_continuous(breaks = seq(0, 9 * 365, 180), expand = expansion(mult = 0, add = 25)) +
